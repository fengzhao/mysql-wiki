...
但是在BP中的页通常被叫做缓存页，因为 `Buffer Pool` 是一个缓冲池，并且里面的数据都是从磁盘文件中缓存到内存中。
...
重做日志，又叫事务日志，是 InnoDB 存储引擎层的日志。有时候也有人叫 **物理日志** 
...
举个简单的例子，假设你非常热心且 rich 的，借出去了很多钱，但是你并且记性不太好，所以非常 old school，不会使用电子设备你用一个小本本记下了所有欠你钱的人的名字和具体金额。这样，别人还你钱的时候，你就翻出你的小本本，一页页地找到他的名字然后把这次还的钱扣除掉。
...
> PostgreSQL 官方文档中也有这么一句话 Write-Ahead Logging (WAL) is a standard method for ensuring data integrity

==通过日志实现事务的原子性和持久性是当今的主流方案，在各种数据系统中都可以体现出来==
...
# binlog文件(直接带全路径)，最后生成的就是mysql.000001和mysql.000002这样的文件
log_bin=/data/mysql/binlog/mysql-binlog
# binlog文件的索引
log_bin_index=/data/mysql/binlog/mysql-binlog.index
```
...
- 平衡存储空间和性能： 填充因子需要在存储空间利用率和性能之间找到平衡点。


`B+TREE`总是会保持一个平衡，那填充因子到底在这里起一个什么作用，InnoDB在创建或重建b树索引时是进行批量加载的，这样创建索引的途径称为排序索引构建。innodb_fill_factor配置选项定义在排序索引构建期间填充的每个B-tree页面上的空间百分比，其余空间保留给未来索引增长。
