...
UTF-8 是一种变长编码，它的设计目的是为了兼容 ASCII (节省空间) 同时能表示所有 Unicode 字符。

| **字符类型**          | **字节数**   | **示例字符**                                                 |
| --------------------- | ------------ | ------------------------------------------------------------ |
| **基础 ASCII 字符**   | **1 个字节** | 英文字母 (A-Z, a-z)、数字 (0-9)、基础符号 (!, ?, #)          |
| **扩展拉丁字符**      | **2 个字节** | 带有音标的拉丁字符 (如 $\ddot{u}, \acute{e}$)                |
| **中日韩 (CJK) 字符** | **3 个字节** | 汉字 (如 "中", "文")、日文假名、韩文                         |
| **生僻字符/特殊符号** | **4 个字节** | 一些罕见或生僻的汉字、特殊数学符号、Emoji 表情符号 (如 🚀, 😂) |
...
| **排序规则 (Collation)** | **utf8mb4_0900_ai_ci**                                       | **utf8mb4_unicode_ci**                                       | **utf8mb4_general_ci**                                     |
| ------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ---------------------------------------------------------- |
| **基础算法**             | **UCA 9.0.0**                                                | **UCA 4.0.0 或更早**                                         | **自定义/简化规则**                                        |
| **准确性**               | **最高 (基于最新 Unicode 标准)**                             | 较高 (基于旧 Unicode 标准)                                   | 一般 (为了速度而妥协准确性)                                |
| **性能/速度**            | **较快**                                                     | 较慢                                                         | **最快 (通常)**                                            |
| **特点**                 | 遵循最新的 Unicode 标准，支持复杂的多级排序规则。            | 遵循 Unicode 标准，但在处理复杂语言和生僻字时不如 0900 准确。 | 速度优化，实现简单，排序结果可能不符合某些语言的精确规则。 |
| **是否推荐**             | **强烈推荐 (MySQL 8.0 默认)**                                | 曾被推荐，现在被 0900 替代。                                 | 如果对准确性要求低且追求极致速度，可考虑。                 |
| **后缀含义**             | **`0900`**: Unicode 版本 9.0.0；**`ai`**: 不区分重音 (Accent Insensitive)；**`ci`**: 不区分大小写 (Case Insensitive)。 | **`unicode`**: 基于 Unicode 算法；**`ci`**: 不区分大小写。   | **`general`**: 通用/简化；**`ci`**: 不区分大小写。         |
...
`CJK Unified Ideographs（中日韩统一表意文字，简称CJK`）是指在ISO 10646和Unicode标准中，为了节省编码空间，将中、日、韩、越等国家和地区起源相同、本质相同、形状相似的汉字（表意文字）合并赋予相同代码的字符集合。该编码范围主要集中在 U+4E00–U+9FFF 基本区，包含了数万个汉字。 

`Unicode`中文字符集中存在两个极为相似的汉字但编码不同的情况，主要是因为康熙部首和扩展部首中存在和基本汉字相似的汉字，如 ：
...
`GB18030`编码采用单字节、双字节、四字节分段编码方案，具体码位见下文。GB18030向下兼容GBK和GB2312编码。

国家标准`GB18030-2005`《信息技术 中文编码字符集》是我国继`GB2312-1980`和`GB13000-1993`之后最重要的汉字编码标准，是我国计算机系统必须遵循的基础性标准之一。

`GB18030`有两个版本：`GB18030-2000`和`GB18030-2005`。

- `GB18030-2000`是GBK的取代版本，它的主要特点是在GBK基础上增加了CJK统一汉字扩充A的汉字。

- `GB18030-2005`的主要特点是在`GB18030-2000`基础上增加了CJK统一汉字扩充B的汉字。
