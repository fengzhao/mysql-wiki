...
???+ 实体
    ...
    例如每个人就是一个实体，人有姓名，性别，年龄，身份证等属性。身份证号就可以唯一标识一个人。可以简单把一个实体等价认为一行数据。
     ...
     - 实体属性：实体通过一组属性来表示，属性是实体集中的每个实体都拥有描述性信息，比如每个student实体都有 student_id，sex，name，age等属性。
     - 简单属性：不可以进一步拆分的属性，例如：student实体的sex属性只能是male或female。
     - 复合属性：可以进一步拆分的属性，例如：student实体的name属性可以进一步拆分为：first_name和last_name两个子属性。
     - 单值属性：每个特定实体中的某个属性只有一个值。即student表中任何一个行的sex列都只能是一个值，male和female。
     - 多值属性：每个特定实体中的某个属性可能有多个值。即student表中任何一个行的phone_number列可能有多个值，一个学生有多个电话号码。
     - 派生属性：这类属性的值可以从别的相关属性或实体派生出来（也就是可通过别的属性计算出来）。即年龄是通过当前时间和出生日期计算出来的。
...
???+ 映射基数
    ...
    映射基数（基数比率）：表示一个实体通过联系集可能关联的实体个数。
     ...
     - 一对一：A实体集中的一个实体最多与B实体集中一个实体关联，B实体集中的一个实体最多与A实体集中一个实体关联。每个学生只能有一个老师，每个老师只能有一个学生。
     - 一对多：A实体集中的一个实体可能与B实体集中任意个实体关联，B实体集中的一个实体最多与A实体集中一个实体关联。每个学生可以有任意个老师，每个老师只能有一个学生。
     - 多对一：多个学生对应一个老师。**<br>
...
- ==关系永远也不会包含重复的元组。这一性质是成立的，因为主体的定义是元组的集合，而数学上的集合不包含重复元素。然而 `SQL` 不是这样的，一般情况下 SQL 表允许包含重复行（SQL 允许无主键）。==
...
完整性约束用于维护数据的完整性或者满足业务约束的需求，包括

- **实体完整性（主键约束）**
- **参照完整性（外键约束）**
- **用户定义的完整性（非空约束、唯一约束、检查约束和默认值）**
...
???+ 部分函数依赖
    ...
    要判断依赖关系是"完全"还是"部分"，核心在于：属性 Y 到底是依赖于完整的 X，还是仅仅依赖于 X 的一部分？

    Y 虽然表面上依赖于 X，但实际上它只依赖于 X 里的一部分属性。X 里包含了多余的信息。

???+ 完全函数依赖

    Y 依赖于 X 的整体。只有把 X 里的所有属性凑齐了，才能确定 Y。缺了任何一个属性，都无法确定 Y。
...
???+ 第一范式
    ...
    在当前的任何关系数据库管理系统 RDBMS 中，设计出不符合第一范式的数据库都是不可能的。因为这些 DBMS 不允许你把数据库表的一列再分成二列或多列。
...
???+ 第二范式
    ...
    ==非主属性（除主码以外的所有属性）必须完全函数依赖于主键，不能存在部分函数依赖。==
...
???+ 第三范式
    ...
    3NF 要求，表中的所有非主属性，都必须直接依赖于整个主键，不能间接依赖于主键之外的其他非主属性。

    在数据库设计中，传递依赖通常被视为需要消除的设计缺陷（除非你有特殊的性能需求）。因为它会导致以下几个严重问题（统称为“异常”）：

        A. 数据冗余（Data Redundancy）
        如果计算机系有 5000 个学生，那么在表中，“计算机系 - 张教授” 这组信息就会重复存储 5000 次。这不仅浪费空间，还很难看。

        B. 修改异常（Update Anomaly）
        如果“张教授”退休了，换成了“李教授”。你需要把这 5000 个学生的记录全部改一遍。如果你漏改了某一条，数据库就会出现“计算机系的主任既是张教授又是李教授”的逻辑错误。

        C. 删除异常（Deletion Anomaly）
        假设计算机系刚刚成立，还没有学生招进来（或者所有学生都毕业了，删除了学生记录）。因为“系主任”是跟“学生”存在同一张表里的，如果学生记录没了，“计算机系的主任是张教授”这条信息也会凭空消失。
