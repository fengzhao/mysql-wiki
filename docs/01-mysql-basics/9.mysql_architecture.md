...
CPU和IO其实是两种不同的资源，那么假设执行计划A的CPU Cost低，但IO Cost高，而执行计划B的CPU Cost高，但IO Cost低，这种情况我们应该如何抉择？
...
| **分类**        | **参数名称**                   | **说明**                            | **影响场景**                                          |
| --------------- | ------------------------------ | ----------------------------------- | ----------------------------------------------------- |
| **临时表 (IO)** | `memory_temptable_create_cost` | 创建**内存**临时表的成本            | `GROUP BY`, `DISTINCT`, `UNION` 等                    |
| **临时表 (IO)** | `memory_temptable_row_cost`    | 在内存临时表中读/写一条记录的成本   | 涉及小规模数据的中间结果集操作                        |
| **临时表 (IO)** | `disk_temptable_create_cost`   | 创建**磁盘**临时表的成本            | 当内存临时表溢出到磁盘时的额外开销                    |
| **临时表 (IO)** | `disk_temptable_row_cost`      | 在磁盘临时表中读/写一条记录的成本   | 涉及大规模数据（超过 `tmp_table_size`）的处理         |
| **读取 (IO)**   | `io_block_read_cost`           | 从磁盘随机读取一个 Page 的成本      | **物理 IO**。值越高，优化器越倾向于走全索引扫描或缓存 |
| **读取 (IO)**   | `memory_block_read_cost`       | 从 Buffer Pool 读取一个 Page 的成本 | **逻辑 IO**。通常默认值较低，体现了内存性能优势       |
| **计算 (CPU)**  | `row_evaluate_cost`            | 扫描/评估一条记录的成本             | `WHERE` 条件过滤、全表扫描的代价计算核心              |
| **计算 (CPU)**  | `key_compare_cost`             | 比较索引键值（Key）的成本           | `ORDER BY` 排序、B+树查找、索引合并等                 |



这些常数其实都没有单位，但给他们设置的值要保证是可以比较的。

例如：1的CPU Cost和1的IO Cost在数据库的正常负载下，对于数据库SQL执行速度的影响是相同的。

可以发现，这些常数值对于执行计划的选择非常关键，但却十分难以设置。在大多数情况下，不会去修改这些常数的值。
...
- 高函数调用开销： 每次返回一行数据都需要进行一次函数调用 (Next())。在大数据量下，这种调用开销（CPU 缓存未命中、指令分支预测失败）累积起来会非常显著。

- 效率低下： 每次处理单行数据，无法充分利用现代 CPU 的 SIMD（单指令多数据） 指令集进行并行计算。

为了解决火山模型的痛点，现代的分析型数据库（OLAP）开始采用向量化执行模型。

核心思想：批处理 (Batch Processing)执行方式：操作符不再处理单行数据，而是处理一个由 1000 到 4096 行组成的数据块（Batch/Vector）。

优势：

- 减少函数调用开销：以前需要 1000 次 Next() 调用，现在只需要 1 次。

- SIMD 优化： 可以在整个数据块上使用 SIMD 指令进行计算，将 CPU 利用率推到极致。

采用向量化模型的代表

- 现代分析型数据库： ClickHouse, DuckDB 等原生设计就是向量化的。

- 主流数据库的 OLAP 增强： MySQL 8.0/9.0 的某些功能、SQL Server 的列存储索引和批处理模式、PostgreSQL 的某些扩展（如 Citus）都在查询执行中引入了向量化技术。
...
外连接（如 `LEFT JOIN`, `RIGHT JOIN`, `FULL JOIN`）的复杂性在于，它必须保留那些在连接条件中没有找到匹配项的行，并用 NULL 填充缺失的列。

这种行为给查询执行带来了额外的限制和开销。
...
可以看到 **左外连接的结果集是内连接的超集** ，它主要是增加了R表补空产生的行。
...
SELECT t1.*,t3.* FROM t1 JOIN t2 ON (t1.a1=t2.a2) JOIN t3 ON (t2.a2=t3.a3) ;

-- 可以人工改写成如下
SELECT t1.*,t3.* FROM t1 JOIN t3 ON (t1.a1=t2.a3) ;
