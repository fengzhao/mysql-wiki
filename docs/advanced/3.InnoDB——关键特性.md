...
在PostgreSQL中默认page大小为8KB，在MySQL中默认page大小为16KB，而操作系统的单次IO一般以4KB为单位进行读写，所以写完WAL日志（or redo log）后，在往磁盘写数据页的过程中如果一个page写到一半出现了问题，那么下次启动在根据 WAL 日志恢复时，再基于这个`corrupted page`进行恢复肯定是不行的。
