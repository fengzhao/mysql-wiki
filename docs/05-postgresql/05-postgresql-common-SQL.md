...
在PostgreSQL中，==不带引号的标识符（例如表名、列名等）总是会被折叠成小写==。例如标识符FOO、foo和"foo"在PostgreSQL中被认为是相同的。

但是"Foo"和"FOO"与这三个不同，并且彼此不同。因为他们是带了引号的标识符。

```SQL

-- 即使这里建表语句定义为"DB_TEST"。在PG内部，表名仍然会变成"db_test"
...
**再强调一遍，关键词和不被引号修饰的标识符（无论是表名，列名等等）是大小写不敏感的**
...
??? note "数据库创建"
    ...
    | ------------ | ------------------------------------------ | ---------------------------------------- |
    | **定位**     | **用户级**默认模板                         | **系统级**核心模板                       |
    | **主要用途** | 供用户自定义，存放通用的表、扩展、函数。   | 保持“出厂设置”，作为最后的恢复手段。     |
    | **可变性**   | 允许用户连接并修改（加表、建索引等）。     | **严禁修改**。它是数据库实例的物理基石。 |
