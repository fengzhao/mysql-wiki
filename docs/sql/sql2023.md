...
SQL 标准每隔几年就会发布一次，国际标准化组织（ISO）于2023 年6 月1 日正式发布了最新SQL 标准，也就是`SQL:2023`。Peter Eisentraut在这篇文章有详细的示范说明：
...
- As of this writing, PostgreSQL 16 is in feature freeze but not released yet. So those entries could in theory change, but it’s pretty unlikely. Conformance information for PostgreSQL 16 relative to SQL:2023 can also be found in the [documentation](https://www.postgresql.org/docs/devel/features.html).
