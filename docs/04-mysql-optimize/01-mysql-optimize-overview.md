...
[TPC-C](https://www.tpc.org/tpcc/)是TPC（事务处理性能委员会）推出的一系列性能测试标准中的一款，自1992年推出，便成为了数据库性能测试的标杆，各个数据库大厂都向TPC委员会提交了测试结果，以期在TPC-C测试的排行榜上拥有一席之地。

就像决战紫禁之巅，论剑华山之巅，拥有一席之地的都是成名的大佬们。
...
TPC-C模型是模拟一个商品批发公司的销售模型，这个模型涵盖了一个批发公司面向客户对一系列商品进行销售的过程，这包括管理订单，管理库存，管理账号收支等操作。

这些操作涉及到仓库、商品、客户、订单等概念，围绕这些概念，构造了数据表格，以及相应的数据库操作。
...
TPC-C的批发操作在概念上包括5个操作，新订单操作、付款操作、订单状态操作、发货操作，库存操作，这些操作一起构成了完整的批发流程，其中新订单操作是用来衡量tpmC的核心，也就是数据库系统每分钟所能处理的交易数量。

下面，我们将详细讲述TPC-C的数据库设计与特点，以及各个操作的组成。



| **业务事务**                | **模拟场景**                         | **操作类型**        | **业务配比** | **技术挑战与考察重点**                                       |
| --------------------------- | ------------------------------------ | ------------------- | ------------ | ------------------------------------------------------------ |
| **New-Order** (新订单)      | 客户放置一个包含 5-15 条目的新订单   | **读写混合 (高频)** | **~45%**     | **核心性能指标：** 考察高并发下的行插入（Order表）和索引更新能力。 |
| **Payment** (付款)          | 更新客户余额、仓库及区域累计收入     | **读写混合**        | **~43%**     | **锁竞争：** 涉及对 Warehouse/District 表热点行的频繁更新，测试锁管理机制。 |
| **Order-Status** (订单查询) | 查询指定客户最近一次订单的状态       | **只读**            | **~4%**      | **索引效率：** 考察数据库在大数据量下通过非主键索引（如客户名）检索的能力。 |
| **Delivery** (发货)         | 批量处理待发货订单，更新状态及日期   | **写操作 (批处理)** | **~4%**      | **长事务处理：** 涉及多行更新与删除，测试系统在背景运行重负载事务时的稳定性。 |
| **Stock-Level** (库存检查)  | 检查最近 20 个订单中库存不足的商品数 | **只读 (聚合)**     | **~4%**      | **复杂查询：** 涉及多表 Join（连接）和聚合计算，测试 OLTP 对轻量分析的响应。 |



TPC-C使用**tpmC**值（Transactions per Minute）来衡量系统最大有效吞吐量（MQTh，Max Qualified Throughput），其中Transactions以`NewOrder Transaction`为准，即最终衡量单位为每分钟处理的新订单数。
...
**==tpmC流量指标(Transactions per minute C)==**
...
| **维度**          | **TPC-C (传统交易)**      | **TPC-E (现代交易)**             | **TPC-H (分析决策)**         |
| ----------------- | ------------------------- | -------------------------------- | ---------------------------- |
| **应用领域**      | **OLTP** (联机事务处理)   | **OLTP** (联机事务处理)          | **OLAP** (联机分析处理)      |
| **模拟场景**      | 批发商/仓储订单管理       | 证券交易所/经纪人业务            | 决策支持/数据仓库            |
| **核心特点**      | 简单、高并发、写密集      | **复杂、高度关联、真实分布**     | 极复杂、全表扫描、计算密集   |
| **表数量**        | 9 张表                    | **33 张表**                      | 8 张表                       |
| **事务/查询类型** | 5 种事务 (新订单为核心)   | 12 种事务 (包含行情查询)         | 22 条复杂的分析 SQL          |
| **数据读写比**    | 约 1.9 : 1 (写操作非常重) | **约 19 : 1** (模拟频繁查股价)   | 几乎纯读取 (只读分析)        |
| **数据分布**      | 均匀分布 (理想化模型)     | **非均匀分布** (模拟真实市场)    | 大规模、线性增长             |
| **核心度量指标**  | **tpmC** (每分钟新订单数) | **tpsE** (每秒交易数)            | **QphH** (每小时查询数)      |
| **主要考验能力**  | 锁竞争、日志写入、缓冲池  | **索引优化、执行计划、复杂约束** | 并行计算、Join 性能、IO 吞吐 |
