...
**在一些操作系统中，直接在 mysql 命令中指定密码是不安全的，其他用户可以通过 ps 等命令显示出你输入的明文密码。所以在终端中，尽量避免使用 --password=password 和 -ppassword 这种格式。**

在一些操作系统中，限制了在终端中输入的密码数最大是8位字符，这是系统限制，并不是 mysql 限制。在 mysql 内部并没有密码长度的限制。为了解决这类问题，把密码调整到符合系统限制或者放到配置文件中即可。



mysql_config_editor可以给指定的连接和密码生成一个加密文件.mylogin.cnf，默认位于当前用户家目录下。mysql、mysqladmin等可以使用该文件直接登录，避免明文密码出现在脚本中。
