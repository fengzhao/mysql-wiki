...
有时在修改行的操作过程中获取数据很有用。INSERT、 UPDATE和DELETE命令都有一个支持这个的可选的 RETURNING子句。
...
目前只有 Oracle 和 SQL Server 支持 MERGE 语句，`PostgreSQL 15` 已经确认会增加 MERGE 语句。在此之前，我们可以使用 `INSERT … ON CONFLICT DO …` 语句代替该功能。
