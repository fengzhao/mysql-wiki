...
窗口排序主要是指非全局排序，需要在某个维度下进行排序。

例如说现在想看各个department内部cost最多的人，这时候不能全局order by了。分组再组内排序。
...
窗口函数（Window Function）是 SQL2003 标准中定义的一项新特性，并在 SQL2011、SQL2016 中又加以完善，添加了若干处拓展。窗口函数不同于我们熟悉的普通函数和聚合函数，它为每行数据进行一次计算：==输入多行（一个窗口）、返回一个值==。

在报表等分析型查询中，窗口函数能优雅地表达某些需求，发挥不可替代的作用。
