...
在标准SQL中，SQL语言国际标准中，`SAVEPOINT name`语句声明一个savepoint。`ROLLBACK TO SAVEPOINT name`语句回滚到savepoint。`RELEASE SAVEPOINT name`将使得命名的savepoint被放弃，但不影响其他savepoint。`ROLLBACK`或`COMMIT`导致所有`savepoint`被放弃。
